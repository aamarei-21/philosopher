...
/*   Updated: 2021/07/18 19:33:07 by aamarei          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pars_arg(int c, t_phil_data *phil, char **v)
{
	...
	if (c == 6)
	{
		...
		return (-1);
	phil->prin = malloc(sizeof(sem_t) * (phil->num_phil + 1));
	phil->prin = sem_open("print", O_CREAT, 0600, 1); //!обработать ошибку
...
void	ft_init_phil(t_philos **phil, t_phil_data *tm, sem_t *forks)
{
	...
	*phil = (t_philos *)malloc(sizeof(t_philos) * (tm->num_phil + 1));
	forks = sem_open("forks", O_CREAT, 0600, tm->num_phil); //!Обработать ошибку
	while (++i <= tm->num_phil)
	{
		//(*phil)[i].num = i;
		//(*phil)[i].right = i;
